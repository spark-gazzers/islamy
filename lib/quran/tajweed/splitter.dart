...
class AyahTajweedSplitter {
  /// Splits the [Ayah]'s [text] and returns [List<TextSpan>] compined represents the respective ayah script,
  /// colored with the tajweed rules
  ///
  /// Coloring and identifying the tajweed rules comes for specific [Edition] and
  /// The rules are stored at [TajweedRules].
  ///
  /// Thus added as plane spans or colored spans will not modify any other property from the [TextStyle]
  /// besides the color
  ///
  ...
  static List<TextSpan> formatAyah(Ayah ayah) {
    ...
    int index = 0;
    // The loop algorithem loops for each `[` in the string which is not mentioned in the quran but are used
    // to mark each letter (that needs) with it's specific tajweed guide line
    do {
      ...
      if (newIndex == -1) {
        ...
        String id = text.substring(newIndex, newIndex + 2);
        // Respected tajweed rule from the id
        TajweedRule rule =
            TajweedRules.ids.singleWhere((element) => element.id == id);
        String letterToPatch = text.substring(
            text.indexOf('[', newIndex + 1) + 1, text.indexOf(']', newIndex));
      ...
      // if the index pointing to the end do nothing else add 1 to start from the next position
